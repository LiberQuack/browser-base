...
sleep 1 && ffmpeg -f x11grab -video_size $RESOLUTION -i $DISPLAY -codec:v libx264 -r $FRAME_RATE -y "${VIDEO_DIRECTORY}/${RECORDED_FILE_NAME}.${VIDEO_EXTENSION}" >& /tmp/ffmpeg.log &
sleep 1 && exec "$@"
